...
function App() {
  ...
  // submit handler to slice paragraphs according to input value
    ...
    <Fragment>
      <div className="container"> <h1>TIRED OF BORING LOREM IPSUM?</h1>
        ...
        <form onSubmit={(e) => submithandler(e)}>
          ...
          <button className="generate">Generate</button></form>

        <article> {text.map(item => {
          return (<p key={Math.random()} className="text">{item}</p>)
        })}</article>
